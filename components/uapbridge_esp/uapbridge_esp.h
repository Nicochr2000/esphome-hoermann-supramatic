#pragma once

#include "esphome/core/component.h"
#include "esphome/core/helpers.h"
#include "esphome/core/log.h"
#include "esphome/components/uart/uart.h"
// ESP-IDF only component (see __init__.py -> cv.only_with_esp_idf).
// We talk to the ESP-IDF UART driver directly to change the baud rate / word
// length on the fly WITHOUT going through ESPHome's UARTComponent::load_settings(),
// which reinstalls the whole driver since ESPHome 2026.2.3 (see uapbridge_esp.cpp).
#include "driver/uart.h"
#include "esphome/components/uart/uart_component_esp_idf.h"
#include "esphome/components/uapbridge/uapbridge.h"

// Main loop pacing (milliseconds)
#define CYCLE_TIME                1    // how often we may transmit a queued bus answer
#define CYCLE_TIME_SLOW         100    // how often we re-evaluate the decoded door state

// HCP/UAP bus addressing
#define BROADCAST_ADDR            0x00
#define UAP1_ADDR                 0x28
#define UAP1_ADDR_MASTER          128

#define UAP1_TYPE                 0x14

// HCP/UAP bus commands
#define CMD_SLAVE_SCAN            0x01
#define CMD_SLAVE_STATUS_REQUEST  0x20
#define CMD_SLAVE_STATUS_RESPONSE 0x29

namespace esphome {
namespace uapbridge_esp {

/**
 * ESP32 implementation of the Hörmann UAP1 emulation.
 *
 * The class plugs onto the door drive's RS485 "HCP2" bus (19200 baud, 8N1) and
 * pretends to be a genuine Hörmann UAP1 universal adapter. It decodes the
 * broadcast status frames sent by the drive and answers the drive's slave-scan
 * and slave-status-request polls so the drive keeps trusting the (virtual) UAP1.
 */
class UAPBridge_esp : public esphome::uapbridge::UAPBridge {
 public:
  // Commands that can be sent back to the drive inside a "slave status response".
  // The 0x10xx high byte is constant; the low byte selects the action.
  enum hoermann_action_t {
    hoermann_action_stop          = 0x10FF,  // see https://github.com/avshrs/ESP32_Hormann_Supramatic_e3
    hoermann_action_open          = 0x1001,
    hoermann_action_close         = 0x1002,
    hoermann_action_impulse       = 0x1004,
    hoermann_action_toggle_light  = 0x1008,
    hoermann_action_venting       = 0x1010,
    hoermann_action_none          = 0x1000,
  };

  void setup() override;
  void loop() override;

  // High level actions (called by the cover / buttons / switches)
  void action_open();
  void action_close();
  void action_stop();
  void action_venting();
  void action_toggle_light();
  void action_impulse();

  hoermann_state_t get_state();
  std::string get_state_string();
  void set_venting(bool state);
  void set_light(bool state);

 protected:
  hoermann_state_t state = hoermann_state_stopped;
  hoermann_action_t next_action = hoermann_action_none;
  std::string state_string = "unknown";

  // --- Internal helpers ---
  void loop_fast();
  void loop_slow();
  void receive();
  void transmit();
  void set_command(bool cond, const hoermann_action_t command);
  uint8_t calc_crc8(uint8_t *p_data, uint8_t length);
  void handle_state_change(hoermann_state_t new_state);
  char *print_data(uint8_t *p_data, uint8_t from, uint8_t to);
  void update_boolean_state(const char *name, bool &current_state, bool new_state);

  // Cached ESP-IDF UART port number of the bus, resolved in setup().
  uart_port_t uart_port_{UART_NUM_0};

  // --- Internal state ---
  uint32_t last_call = 0;
  uint32_t last_call_slow = 0;
  uint16_t broadcast_status = 0;
  bool ignore_next_event = false;  // also suppresses a wrong edge right after we sent a command
  bool auto_correction_in_progress = false;
  uint8_t rx_data[5] = {0, 0, 0, 0, 0};
  uint8_t tx_data[6] = {0, 0, 0, 0, 0, 0};
  uint8_t tx_length = 0;
  uint8_t byte_cnt = 0;
  uint32_t send_time = 0;

  // Pre-computed CRC8 table (polynomial 0x07, init 0xF3)
  const uint8_t crc_table[256] = {
      };
};

}  // namespace uapbridge_esp
}  // namespace esphome
